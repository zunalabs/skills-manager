/** Renders the appropriate icon for a given AI tool name. */
export function ToolIcon({ tool, size = 24 }: { tool: string; size?: number }) {
  if (tool === 'Claude Code') {
  ...
  if (tool === 'Goose') {
    return <img src="/src/goose.png" width={size} height={size} style={{ objectFit: 'contain' }} alt="Goose" />
  }
  if (tool === 'GitHub Copilot') {
    return <GitHubCopilotIcon size={size} />
  }
  if (tool === 'Kilo Code') {
    return <KiloCodeIcon size={size} />
  }
  ...
  if (tool === 'Trae') {
    return <TraeIcon size={size} />
  }

  if (tool === 'Codex') {
    return <img src="/src/codex.png" width={size} height={size} style={{ objectFit: 'contain' }} alt="Codex" />
  }

  return (
    <span style={{ fontSize: size * 0.75 }} className="leading-none">🔧</span>
  )
}
...
function AntigravityIcon({ size }: { size: number }) {
  // Renders just the colorful symbol portion of the Antigravity wordmark (approx square crop)
  return (
    <svg
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    ...
    >
      <defs>
        ...
        <mask id="ag-mask" maskUnits="userSpaceOnUse" x="13" y="18" width="85" height="78" style={{ maskType: 'alpha' }}>
...
function GitHubCopilotIcon({ size }: { size: number }) {
  // First frame cropped from the official Copilot idle sprite (viewBox targets y=808–840)
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
...
function CursorIcon({ size }: { size: number }) {
  // Hexagonal "C" logo — cropped from the Cursor wordmark SVG (first path only)
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
...
function WindsurfIcon({ size }: { size: number }) {
  // Crop just the wave "W" mark from the wordmark (first path, x=0–232)
  return (
